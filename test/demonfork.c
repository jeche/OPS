...
main()
{
  /* Runs a demonic amount of forks. i
     Tests that both wideforking and deepforking work
     concurrently.  Forks up to 16*3 processes 16 of which
     are grandparents.*/
  SpaceId kid[16];
  int joinval;
  char *args[1];
  ...
  for(i=0;i<16;i++){
    ...
    joinval = Join(kid[i]);
    if(joinval != 10){
        print("ERROR demon fork exit val incorrect ");
